...
class OAuth2UserServiceImpl(
    ...
    override fun loadUser(userRequest: OAuth2UserRequest): OAuth2User {
        ...
        val properties = (oAuth2User.attributes.getOrDefault("properties", Collections.emptyMap<Any, Any>()) as? Map<String, Any>)
            ?: emptyMap()
        val nickname = (properties["nickname"] as? String) ?: "카카오사용자"

        // 3) Security에서 사용할 attributes를 확장:
        //    - 원본 attributes를 복사한 뒤 provider_id, nickname을 명시적 key로 추가
        val extendedAttributes = HashMap(oAuth2User.attributes)
        extendedAttributes["provider_id"] = providerId
        extendedAttributes["nickname"] = nickname
        ...
        return DefaultOAuth2User(
            Collections.singleton(SimpleGrantedAuthority("ROLE_USER")), // 부여할 권한 목록
